    ...
    return (
        <div className='w-full md:w-1/2 space-y-2 md:space-y-4'>
            <AnimatePresence mode='wait'>
                ...
                >
                    <Image
                        ...
                        height={700}
                        priority
                        className={`w-full h-96 max-h-[550px] min-h-[550px] object-contain group-hover:scale-110 hoverEffect rounded-md ${isStock===0 ? "opacity-50" : ""}`}
                    />
                </motion.div>
            </AnimatePresence>
            ...
            <div className='grid grid-cols-6 gap-2 h-20 md:h-24'>
                {images?.map((image) => (
                    <button
                        className={`border border-muted overflow-hidden ${active?._key === image?._key ? "border-darkColor opacity-100" : "opacity-80"}`}
